...
function getLocaleStrings(locale, template, target, localePaths) {
  ...
  if (locale === "en") {
    return {
      ...
      : isLearningWorkflow
        ? `Keep Current learner brief, roadmap, progress, and validated learning records concise in \`${localePaths.workspaceDir}/\` and formal indexes. Organize complex learning by stable module: each module uses \`index.md\` as its entry point and \`overall-plan.md\` as its Current authority source; create a specialized child document only for an independent lifecycle, audience, review path, or change cadence. After learner confirmation, organize superseded versions and completed process material under \`${localePaths.workspaceDir}/archive/\` by category, topic, and version. Before creating or changing a roadmap, lesson, practice plan, or project plan, present a learning agreement and wait for explicit confirmation; then teach one validated unit at a time.`
        : isWebDesignStandard
  ...
  return {
    ...
    ruleConfirmLargeChanges: isIdeaToProjectWorkflow
      ...
      : isWebDesignStandard
        ...
        : "大范围变更或不可逆操作前，先向用户确认。",
      ruleKeepKnowledge: isIdeaToProjectWorkflow
      ? `将当前事实精简地保留在 \`${localePaths.workspaceDir}/\` 和正式索引中。复杂工作按稳定模块组织：每个模块以 \`index.md\` 为入口、以 \`整体方案.md\` 为当前权威来源；只有内容具备独立生命周期、读者、评审路径或变更节奏时，才新建专门子文档。把暂不推进但可重启的方向记录在 \`${localePaths.workspaceDir}/搁置想法.md\`，写明搁置原因和重启条件，它既不是待确认问题也不是归档材料。用户确认后，才将已替代版本和已结束过程材料按分类、主题和版本放入 \`${localePaths.workspaceDir}/归档/\`。用户明确确认进入项目之前，不得开始完整项目设计、计划或实施；确认后将方向结论同步到项目简报，并使用后续编号目录推进。`
      : isIdeaWorkflow
      ? `把想法简报、方向、假设和决策放在 \`${localePaths.workspaceDir}/\`。先充分发散，再区分事实、假设和证据；收敛到优先方向前等待用户确认。`
    : isLearningWorkflow
      ? `将当前学习简报、课程路线、进度和已验证学习记录精简地保留在 \`${localePaths.workspaceDir}/\` 和正式索引中。复杂学习按稳定模块组织：每个模块以 \`index.md\` 为入口、以 \`整体方案.md\` 为当前权威来源；只有材料具备独立生命周期、读者、评审方式或变更节奏时，才新建专门子文档。学习者确认后，才将已替代版本和已结束过程材料按分类、主题和版本放入 \`${localePaths.workspaceDir}/归档/\`。生成或变更课程路线、章节内容、练习计划或项目方案前，先给出学习约定并等待学习者明确确认；确认后一次只推进一个经过验证的学习单元。`
      : isWebDesignStandard
