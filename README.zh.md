...
| --- | --- | --- |
| 从想法推进到项目 | 探索、验证、方向决策包、可重启的搁置想法、项目简报、方案、决策、评审 | `idea-to-project` |
...
适合长期项目、代码、复杂学习，以及需要持续沉淀或可审查过程的工作。RecoWork 会在你选择的目录中初始化根目录 `AGENTS.md`、工作方法、适用的本地化工作空间和版本化清单。正式工作文档始终保持当前且精简，确认保留的历史版本统一进入工作空间归档。
...
查看器会在 `127.0.0.1` 启动，并在可用时打开浏览器；它不会写入配置、重新生成文件或修改工作空间。Markdown 或索引被编辑后，它会自动刷新当前文档和导航；同时支持可复制的文档链接、全文搜索、面包屑与上一篇/下一篇导航、GitHub 风格 Markdown，并优先展示当前工作、按需打开本地化归档。本地 Agent 工作流也会要求 Agent 在初始化完成后，或你要求浏览工作空间时自行启动该命令；同一工作空间的查看器已运行时会自动复用，用户无需全局安装、查找工作空间目录或操作终端。需要时使用 `--no-open` 或 `--port 4311`。
